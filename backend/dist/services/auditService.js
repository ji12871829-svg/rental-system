...
async function logAudit(opts) {
    try {
        await (0, db_1.query)(`INSERT INTO audit_logs (user_id, action, entity, entity_id, old_value, new_value)
...
async function listAuditLogs(opts) {
    ...
    const totalRow = await (0, db_1.queryOne)(`SELECT COUNT(*)::text AS count FROM audit_logs a ${whereSql}`, params);
    const total = Number(totalRow?.count ?? 0);
    const offset = (opts.page - 1) * opts.limit;
    const rows = await (0, db_1.query)(`SELECT a.*, u.name AS user_name, u.email AS user_email
     FROM audit_logs a
     LEFT JOIN users u ON u.id = a.user_id
     ${whereSql}
     ORDER BY a.created_at DESC
     LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, opts.limit, offset]);
    return {
        rows,
