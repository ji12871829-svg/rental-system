...
1. Render Dashboard → **New → Blueprint** → pick this repo. Render reads
   ...
   > The blueprint pins **region: frankfurt** so the service sits next to the
   > Neon database (EU). Do not change it to a different region than the Neon
   > project — every DB query pays a network round-trip between the two. An
   > existing service cannot be moved; create it fresh from the blueprint to
   > pick up the region.
